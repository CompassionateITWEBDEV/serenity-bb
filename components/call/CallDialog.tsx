...
export default function CallDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => v ? onOpenChange(true) : (hangup().catch(() => {}), onOpenChange(false))}>
      <DialogContent className="max-w-3xl overflow-hidden" aria-describedby="call-desc">
        ...
        <div className="mt-3 flex items-center justify-center gap-2">
          {state.status !== "ended" ? (
            <>
              <Button variant={state.muted ? "secondary" : "default"} onClick={() => setMuted(!state.muted)} className="rounded-full" title={state.muted ? "Unmute" : "Mute"}>
                {state.muted ? <MicOff className="h-5 w-5" /> : <Mic className="h-5 w-5" />}
              </Button>
              {mode === "video" && (
                <Button variant={state.camOff ? "secondary" : "default"} onClick={() => setCamOff(!state.camOff)} className="rounded-full" title={state.camOff ? "Turn camera on" : "Turn camera off"}>
            ...
            <>
              <Button variant="secondary" disabled className="rounded-full">Call ended</Button>
