...
export default function PatientBroadcasts({ items }: Props) {
  const [q, setQ] = useState("");
  const [broadcasts, setBroadcasts] = useState<Broadcast[]>(items || []);
  const [loading, setLoading] = useState(false);
  const supabase = createClient();

  // Fetch broadcasts targeted at patients
  useEffect(() => {
    fetchBroadcasts();
    
    // Real-time subscription for new broadcasts
    const ch = supabase
      .channel('rt-patient-broadcasts')
      .on('postgres_changes', { 
        event: 'INSERT', 
        schema: 'public', 
        table: 'broadcasts',
        filter: 'target_audience=in.(all,patients)'
      }, (payload) => {
            ...
            >
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  <div className="h-10 w-10 rounded-full bg-blue-100 text-blue-700 grid place-items-center shrink-0">
