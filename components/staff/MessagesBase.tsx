...
/**
 * Base UI for Messages (Internal Groups + Direct Messages) modeled from the mock.
 * TailwindCSS required. No backend; swap `SEED_*` with your data later.
 * Why: Keep structure identical to mock so wiring later is trivial.
 */
...
const Icons = {
  phone: (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor">
      <path strokeWidth="2" d="M2 5a3 3 0 0 1 3-3h3l2 5-2 1a12 12 0 0 0 6 6l1-2 5 2v3a3 3 0 0 1-3 3h-1C9.82 20.5 3.5 14.18 2 7V5z"/>
    </svg>
  ),
  calendar: (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor">
  ...
  mic: (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor">
      ...
      <path d="M5 10a7 7 0 0 0 14 0M12 19v3" strokeWidth="2"/>
    </svg>
  ),
  bell: (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor">
      <path strokeWidth="2" d="M18 8a6 6 0 1 0-12 0c0 7-3 7-3 9h18c0-2-3-2-3-9"/>
      <path d="M13.73 21a2 2 0 0 1-3.46 0" strokeWidth="2"/>
    </svg>
  ),
  filter: (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor">
      <path strokeWidth="2" d="M3 5h18M7 12h10M10 19h4"/>
    </svg>
  ),
  search: (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor">
  ...
  check: (
    <svg viewBox="0 0 24 24" className="h-3 w-3" fill="none" stroke="currentColor">
      <path strokeWidth="3" d="M20 6 9 17l-5-5"/>
    </svg>
  ),
  plus: (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor">
  ...
  people: (
    <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor">
      <path strokeWidth="2" d="M16 11a4 4 0 1 0-8 0M3 21a7 7 0 0 1 18 0"/>
    </svg>
  ),
  inbox: (
    <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor">
      <path strokeWidth="2" d="M20 7H4l-2 6 4 6h12l4-6-2-6z"/>
      <path strokeWidth="2" d="M2 13h6a4 4 0 0 0 8 0h6"/>
    </svg>
  ),
  settings: (
    <svg viewBox="0 0 24 24" className="h-5 w-5" fill="none" stroke="currentColor">
      <path strokeWidth="2" d="M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z"/>
      <path strokeWidth="2" d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V22a2 2 0 1 1-4 0v-.07a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06A2 2 0 1 1 3.2 19.7l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H2a2 2 0 1 1 0-4h.07a1.65 1.65 0 0 0 1.51-1 1.65 1.65 0 0 0-.33-1.82l-.06-.06A2 2 0 1 1 5.98 3.2l.06.06c.5.5 1.23.65 1.82.33A1.65 1.65 0 0 0 9.37 2H9.5a2 2 0 1 1 4 0h.07c.62 0 1.2.23 1.64.65.59.32 1.32.17 1.82-.33l.06-.06A2 2 0 1 1 20.8 5.98l-.06.06a1.65 1.65 0 0 0-.33 1.82c.25.58.82.97 1.51 1H22a2 2 0 1 1 0 4h-.07c-.69 0-1.26.39-1.51 1z"/>
    </svg>
  ),
};
...
const Avatar: React.FC = () => (
  <div className="h-10 w-10 rounded-full bg-teal-100 grid place-items-center text-slate-500">
...
const StatusDot: React.FC<{ type?: "ok" | "dot" | "none" }> = ({ type }) => {
  ...
  const base =
    "h-5 w-5 rounded-full grid place-items-center border border-transparent";
  return type === "ok" ? (
...
const ThreadRow: React.FC<{ item: ThreadItem }> = ({ item }) => (
  <button
  ...
  >
    ...
    <div className="flex-1 min-w-0">
      ...
      <div className="text-xs text-slate-500 clamp-2 leading-snug">
    ...
    <div className="card bg-white/90 rounded-2xl p-4 space-y-4">
      ...
      <div className="divide-y divide-slate-100">
        {filtered.length === 0 ? (
          ...
          filtered.map((t) => <ThreadRow key={t.id} item={t} />)
        )}
      </div>
...
const SEED_GROUPS: ThreadItem[] = [
  ...
  },
  {
    ...
    kind: "group",
    name: "Robert Mccloud",
    subtitleTop: "Therapy ready. Full care team.",
    subtitleBottom: "Dr. Parker – MPB PPO",
    rightStatus: "none",
  },
  {
    id: "g3",
    kind: "group",
    name: "Rhonda Fairley",
    subtitleTop: "Wound care ongoing. SN only.",
    subtitleBottom: "Dr. Packey – MCARE ADV",
    rightStatus: "dot",
  },
  {
    id: "g4",
    kind: "group",
    name: "Emma McElroy",
    subtitleTop: "Auth hold. No new updates.",
    subtitleBottom: "Dr. Palfy – Wellcare HMO",
    rightStatus: "none",
  },
  {
    id: "g5",
    kind: "group",
    name: "Adrian Foster",
    subtitleTop: "Vitals improved after visit.",
    subtitleBottom: "Dr. Moore – United HealthCare",
    rightStatus: "none",
  },
  {
    id: "g6",
    kind: "group",
    name: "Lana Jenkins",
    subtitleTop: "Medication adjusted today.",
    subtitleBottom: "Dr. Patel – Aetna Gold Plan",
    rightStatus: "ok",
  },
];
...
const SEED_DMS: ThreadItem[] = [
  {
    ...
    kind: "dm",
    name: "Bria Patterson",
    subtitleTop: "Happy Friday team!!!",
    subtitleBottom: "Thanks for all You...",
    rightStatus: "dot",
  },
  {
    id: "d2",
