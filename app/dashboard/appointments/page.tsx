...
export default function AppointmentsPage() {
  ...
  // IMPORTANT: use auth.users.id. Patients.user_id references auth.users(id).
  ...
  /* Pop Smart Alerts via SweetAlert */
  useEffect(() => {
    const run = async () => {
      const toShow = alertModels.filter(a => isVisible(a.id)).filter(a => !shownRef.current.has(a.id));
      for (const a of toShow) {
        shownRef.current.add(a.id);
        const icon: "info" | "warning" | "success" =
          a.tone === "warn" ? "warning" : a.tone === "ok" ? "success" : "info";
        const primary =
          a.action?.kind === "join" ? "Join"
          : a.action?.kind === "edit" ? "Review"
          : a.action?.kind === "scheduled" ? "Mark scheduled"
          : a.action?.kind === "rebalance" ? "Rebalance"
          : a.action?.kind === "book" ? "Book now"
          : "OK";
        const res = await swal({
          icon, title: a.title, html: `<div style="text-align:left">${a.desc}</div>`,
          showConfirmButton: true, confirmButtonText: primary,
          showDenyButton: true, denyButtonText: "Snooze 1h",
          showCancelButton: true, cancelButtonText: "Dismiss",
          reverseButtons: true,
        });
        if (res.isConfirmed) {
          if (a.action?.kind === "join") await swalToast("Open your meeting link", "info");
          else if (a.action?.kind === "edit" && a.action.apptId) {
            const appt = items.find(x => x.id === a.action?.apptId); if (appt) openEdit(appt);
          } else if (a.action?.kind === "scheduled" && a.action.apptId) await updateStatus(a.action.apptId, "scheduled");
          else if (a.action?.kind === "rebalance" || a.action?.kind === "book") setIsBookingOpen(true);
        } else if (res.isDenied) {
          snooze(a.id, 60); await swalToast("Snoozed for 1 hour", "info");
        } else {
          dismiss(a.id); await swalToast("Dismissed", "info");
        }
      }
    };
    if (isAuthenticated && patientId) { void run(); }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isAuthenticated, patientId, items.length, nowIso, JSON.stringify(snoozes), JSON.stringify(Array.from(dismissed)), JSON.stringify(alertModels)]);

  /* ====== Ensure patient row exists to satisfy FK ====== */
  async function ensurePatientExists(userId: string) {
    // Why: appointments.patient_id has FK to patients(user_id). Create minimal patient row if missing.
    const { data, error } = await supabase.from("patients").select("user_id").eq("user_id", userId).maybeSingle();
  ...
  async function createAppt() {
    if (!patientId) { await swal({ icon: "error", title: "No user", text: "You must be logged in." }); return; }
    ...
    try {
      ...
      await swal({ icon: "error", title: "Couldn’t book", text: e?.message ?? "Unknown error" });
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Alerts handled via SweetAlert popups */}
