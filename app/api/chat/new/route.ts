...
export async function POST(req: Request) {
  try {
    // Prefer Authorization: Bearer <jwt>, else fallback to cookie session
    let token = getBearerFromRequest(req);
    if (!token) {
      const { data: sess } = await getServerSupabase().auth.getSession();
      token = sess.session?.access_token ?? null;
    }
    if (!token) {
      return NextResponse.json({ error: "Missing/invalid session" }, { status: 401 });
    }
    ...
    if (providerId === uid) return NextResponse.json({ error: "Cannot chat with yourself" }, { status: 400 });

    // Caller must be patient
    ...
    if (!patient) return NextResponse.json({ error: "Only patients can start a chat" }, { status: 403 });

    // Provider must be active staff
    const { data: staff, error: staffErr } = await supabaseAdmin()
      .from("staff")
      .select("user_id, first_name, last_name, email, role, avatar_url, active")
      .eq("user_id", providerId)
      .maybeSingle<StaffRow>();
    if (staffErr) return NextResponse.json({ error: staffErr.message }, { status: 500 });
    if (!staff || !staff.active) return NextResponse.json({ error: "Provider not found or inactive" }, { status: 404 });

    // Prefill provider fields (prevents a second client update)
    const provider_name =
    ...
    if (upsertErr) {
      // If unique violation or race, fetch existing
