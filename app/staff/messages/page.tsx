...
export default function StaffMessagesPage() {
  ...
  async function fetchAssignedPatients(uid: string) {
    try {
      const v = await supabase.from("v_staff_assigned_patients")
        .select("user_id, full_name, email, avatar").eq("staff_id", uid).order("full_name", { ascending: true });
  ...
  async function sendMessage() {
    ...
    const optimistic: MessageRow = {
      id:`tmp-${crypto.randomUUID()}`, conversation_id:selectedId, patient_id:conv.patient_id,
    ...
    const ins = await supabase.from("messages").insert({
      ...
      sender_id:meId, sender_name:meName, sender_role:meRole, content, read:true, urgent:false
    });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // read ?open=<id> safely (no useSearchParams)
  ...
  useEffect(() => {
    ...
    const msgCh = supabase.channel(`msgs_staff_${meId}`).on(
      ...
      (payload) => {
        ...
        const belongs = convs.some(c => c.id === m.conversation_id); if (!belongs) return;
        setUnreadMap(prev => {
          if (selectedId === m.conversation_id) return prev;
          const curr = prev[m.conversation_id] ?? 0; return { ...prev, [m.conversation_id]: curr + 1 };
        });
      }
    ).subscribe();
  ...
  // thread + realtime for open conversation
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur border-b">
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge variant="secondary" className="gap-1"><Activity className="h-3.5 w-3.5" />{`Live · ${assignedCount} patients`}</Badge>
      ...
      <main className="max-w-7xl mx-auto px-6 py-6 space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="h-9 w-9 rounded-full bg-cyan-100 grid place-items-center">
              <svg viewBox="0 0 24 24" className="h-5 w-5 text-cyan-700" fill="none" stroke="currentColor"><path strokeWidth="2" d="M21 15a4 4 0 0 1-4 4H8l-5 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4v8z" /></svg>
            </span>
        ...
        {/* optional panels */}
        {mode === "both" ? (
          <div className="grid gap-6 md:grid-cols-2"><div className="w-full"><Groups/></div><div className="w-full"><DirectMessages/></div></div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <Card className="md:col-span-1">
            ...
            <CardContent className="p-0">
              <div className="divide-y">
                {convs.map(c=>{
                  const active = selectedId===c.id, un=unreadMap[c.id] ?? 0;
                  return (
          ...
          <Card className="md:col-span-2 flex flex-col">
            {!selectedId ? (
              <CardContent className="flex-1 flex items-center justify-center"><div className="text-sm text-gray-500">Select a conversation</div></CardContent>
            ) : (<>
              ...
              <div className="border-t p-3">
                <div className="flex gap-2">
                  <Textarea placeholder="Type a message…" value={compose} onChange={e=>setCompose(e.target.value)}
            ...
            <div className="p-4">
              ...
              <div className="max-h-80 overflow-y-auto divide-y">
                {filteredPatients.map(p=>(
                  <button key={p.user_id} className="w-full text-left p-3 hover:bg-gray-50 flex items-center gap-3"
                    onClick={async ()=>{
                      const conv = await ensureConversationWithPatient(p.user_id);
                      if (!conv || !meId) return;
                      await fetchConversations(meId); await fetchUnread(meId);
                      setSelectedId(conv.id); setModalOpen(false);
                      if (typeof window !== "undefined") {
                        const url = new URL(window.location.href); url.searchParams.set("open", conv.id);
